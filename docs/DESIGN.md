...
> 生成の前段に戦略選択のステップを挟む構成へ変更した）。STEP4eで、URLを覚えていなくても
> 過去データを探せるよう「作成履歴画面」を追加した。
...
| 5 | **作成履歴画面** (`/history`) | 保存済みの戦略分析・生成コンテンツを一覧表示し、検索・絞り込み・複製・削除ができる。画面1〜3のヘッダーから行き来できる |

ダッシュボード、ブランド管理、チーム管理、分析画面などは作らない。
...
- **STEP4d（再分析時の入力内容引き継ぎ）**: 戦略画面・結果画面の「再分析する」がURLを渡すだけで入力画面へ戻り、フォームが空になっていた問題を修正。`/?editAnalysisId=`・`/?sourceContentId=`から元の`ContentInput`を復元する。
- **STEP4e（作成履歴画面）を追加**: URLを知らないと過去の`StrategyAnalysis`・`GeneratedContent`を開けない問題を解消するため、`/history`で一覧・検索・絞り込み・複製・削除ができるようにした。新しいデータベースは追加せず、既存の`madanai:strategy-analyses`・`madanai:contents`をそのまま参照する（`lib/history-storage.ts`に集約）。詳細はREADMEの「作成履歴画面（/history）」を参照。
